...
from app.app.models.models import Log as LogModel
import os
...
class LogService:
    ...
    async def create_log(self, event: str, user_id: Optional[int] = None, event_description: Optional[str] = None) -> LogModel:
    ...
    async def list_logs(self, offset: int = 0, limit: int = 100) -> List[LogModel]:
    ...
    async def get_log(self, log_id: int) -> LogModel:
        data, count = self.supabase.from_('logs').select("*", count='exact').eq("id", log_id).execute()
        if data[1]:
            return LogModel(**data[1][0])
        return None

    async def get_logs_by_user(self, user_id: int, offset: int = 0, limit: int = 100) -> List[LogModel]:
