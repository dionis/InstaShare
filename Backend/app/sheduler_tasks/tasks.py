...
async def _run_compression_logic(mensaje: str):
    ...
    log_entry = LogCreate(
        ...
        event_description=f"Scheduled compression task started at: {datetime.utcnow().isoformat()}"
    )
    ...
    try:
        ...
        for doc_data in documents_to_compress:
            ...
            file_content_response = await supabase.storage.from_(bucket_name).download(file_in_bucket_path)
            ...
            await supabase.storage.from_(bucket_name).upload(compressed_file_path_in_storage, zip_buffer.getvalue(), {'upsert': 'true'})
    ...
    return f"Scheduled compression task completed at: {datetime.utcnow().isoformat()}"
